...
<!-- 작성한 게시글 -->
<div>	
	<div style="display: inline-block; text-align: right; margin-top: 50px;">
	    <h5 style="display: inline;">작성한 게시글</h5>
	    <input type="button" class="member-btn" value="더보기" onclick="location.href='${pageContext.request.contextPath}/member/memberList'">
	</div>
	<c:if test="${!empty list_write}">
		<table class="striped-table" style="margin-top: 30px;">
			...
			<c:forEach var="comm_w" items="${list_write}">
				<tr>
					...
					<td class="align-center" width="10%">${comm_w.comm_regdate}</td>
				</tr>
			</c:forEach>
		</table>
	</c:if>
...
<!-- 좋아요한 게시글 -->
<div style="display: inline-block; text-align: right; margin-top: 50px;">
    <h5 style="display: inline;">좋아요한 게시글</h5>
    <input type="button" value="더보기" class="member-btn" onclick="location.href='${pageContext.request.contextPath}/member/memberFav'">
</div>
